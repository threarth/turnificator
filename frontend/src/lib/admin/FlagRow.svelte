<!--
  ...
  Props:
    ...
    - onaddchild      : (() => void)|null — aggiungi una fascia (solo sulle root)
-->
<script>
...
<tr class={inModifica ? 'table-warning' : 'editable-row'}
    ...
    {#if inModifica}
        ...
        <td style="white-space:nowrap" on:click|stopPropagation>
            <button class="btn btn-success btn-sm py-0" on:click={onsave}><i class="bi bi-check-lg"></i></button>
            <button class="btn btn-secondary btn-sm py-0" on:click={oncancel}><i class="bi bi-x"></i></button>
        </td>
    {:else}
