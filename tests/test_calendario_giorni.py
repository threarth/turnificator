...
Festivi e superfestivi non sono mai dovuti: lavorarli è sempre un turno in
più, che matura un recupero.
...
def test_la_domenica_e_festiva_e_non_lavorativa(giorni):
    ...
    tipo, lavorativo = giorni.classifica_giorno(
        domenica, {}, {LUN, MAR, MER, GIO, VEN, SAB})
...
def test_il_sabato_e_lavorativo_solo_se_lo_si_dice(giorni):
    ...
    _, con_sabato = giorni.classifica_giorno(sabato, {}, {SAB})
    _, senza_sabato = giorni.classifica_giorno(sabato, {}, {LUN})
...
def test_un_superfestivo_resta_riconoscibile(giorni):
    ...
    tipo, _ = giorni.classifica_giorno(
        data, {'superfestivi': ['2026-12-25']}, set(range(7)))
...
def test_settembre_2026_con_sei_giorni_su_sette(giorni):
    ...
    dovuti = giorni.conta_turni_dovuti(
        MESE, ANNO, {}, {LUN, MAR, MER, GIO, VEN, SAB})
    assert dovuti == 26
...
def test_una_festivita_toglie_un_turno_dovuto(giorni):
    ...
    senza = giorni.conta_turni_dovuti(MESE, ANNO, {}, lavorativi)
    con = giorni.conta_turni_dovuti(MESE, ANNO, festivita, lavorativi)
    assert con == senza - 1
